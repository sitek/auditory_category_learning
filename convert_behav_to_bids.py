...
for rx, filename in enumerate(file_list):
    ...
    stim_df.onset = df['sound_1.started'] - (df['sound_1.started'][1]-first_stim_delay)
    
    # define duration
    stim_df.duration[df.corrAns>0] = df['sound_1.stopped'] - df['sound_1.started'].astype(np.float16)
    #stim_df.duration[df.corrAns>0] = 0.3
    
    # define stimulus type (based on sound file)
    stim_df.trial_type[df.soundfile=='stimuli/di1-aN_48000Hz_pol2_S15filt.wav'] = 'di1-aN'
    stim_df.trial_type[df.soundfile=='stimuli/di1-bN_48000Hz_pol2_S15filt.wav'] = 'di1-bN'
    stim_df.trial_type[df.soundfile=='stimuli/di1-hN_48000Hz_pol2_S15filt.wav'] = 'di1-hN'        
    stim_df.trial_type[df.soundfile=='stimuli/di1-iN_48000Hz_pol2_S15filt.wav'] = 'di1-iN'
    stim_df.trial_type[df.soundfile=='stimuli/di2-aN_48000Hz_pol2_S15filt.wav'] = 'di2-aN'
    stim_df.trial_type[df.soundfile=='stimuli/di2-bN_48000Hz_pol2_S15filt.wav'] = 'di2-bN'
    stim_df.trial_type[df.soundfile=='stimuli/di2-hN_48000Hz_pol2_S15filt.wav'] = 'di2-hN'
    stim_df.trial_type[df.soundfile=='stimuli/di2-iN_48000Hz_pol2_S15filt.wav'] = 'di2-iN'
    stim_df.trial_type[df.soundfile=='stimuli/di3-aN_48000Hz_pol2_S15filt.wav'] = 'di3-aN'
    stim_df.trial_type[df.soundfile=='stimuli/di3-bN_48000Hz_pol2_S15filt.wav'] = 'di3-bN'
    stim_df.trial_type[df.soundfile=='stimuli/di3-hN_48000Hz_pol2_S15filt.wav'] = 'di3-hN'
    stim_df.trial_type[df.soundfile=='stimuli/di3-iN_48000Hz_pol2_S15filt.wav'] = 'di3-iN'
    stim_df.trial_type[df.soundfile=='stimuli/di4-aN_48000Hz_pol2_S15filt.wav'] = 'di4-aN'
    stim_df.trial_type[df.soundfile=='stimuli/di4-bN_48000Hz_pol2_S15filt.wav'] = 'di4-bN'
    stim_df.trial_type[df.soundfile=='stimuli/di4-hN_48000Hz_pol2_S15filt.wav'] = 'di4-hN'
    stim_df.trial_type[df.soundfile=='stimuli/di4-iN_48000Hz_pol2_S15filt.wav'] = 'di4-iN'
    

    # define stimulus soundfile
    stim_df.stim_file = df.soundfile
    ...
    # define onset time (relative to the first stimulus presentation)
    resp_df.onset = df['sound_1.started'] - (df['sound_1.started'][1]-first_stim_delay)
    
    # define duration (arbitrary)
    resp_df.duration = 0.1

    resp_df.response_time = df['key_resp.rt']        
    resp_df.correct_key = df['corrAns']
    resp_df.response_key = df['key_resp.keys']

    # only keep rows with a possible correct answer
    resp_df = resp_df[df.corrAns > 0]
    ...
    # define onset time (relative to the first stimulus presentation)
    fb_df.onset = df['text_2.started'] - (df['sound_1.started'][1]-first_stim_delay)

    # feedback is visible from the onset of text_2 to the onset of jitter_cross_post_fb
    fb_df.duration = df['jitter_cross_post_fb.started'] - df['text_2.started']

    # define feedback presented
    fb_df['feedback'] = np.where(df['key_resp.corr']==1, 'correct', 
                                    (np.where(df.corrAns==0, 'none', 'wrong')))
    
    # only keep rows with a possible correct answer
    fb_df = fb_df[df.corrAns > 0]
