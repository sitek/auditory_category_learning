...
parser = argparse.ArgumentParser(
                ...
                epilog=('Example: python mask_trial_betas.py --sub=FLT02 '
                        '--fwhm=0 --atlas=dseg --space=MNI152NLin2009cAsym '
                        '--stat=beta --model=perrun')
                )
...
for sub_id in [subject_id]: #sub_list_prepend:
    print(sub_id)
    ...
    for mx, mask_descrip in enumerate(roi_list):
        ...
        masks_dir = os.path.join(nilearn_dir, 'masks', 'sub-{}'.format(sub_id), 
