...
def prep_models_and_args(subject_id=None, task_label=None, fwhm=None, 
                         bidsroot=None, deriv_dir=None, event_type=None, 
    ...
    ''' add confounds to events list '''
    # grab the limited confound list 
    confounds_ltd = [models_confounds[midx][cx][conf_keep_list] 
                     for cx in range(len(models_confounds[midx])) 
                     for midx in range(len(models))]
    
    # convert from TRs to seconds 
    from scipy.signal import resample
    confounds_resampled = [resample(confounds_ltd[midx][cx], t_r * len(confounds_ltd[midx][cx])) 
                           for cx in range(len(confounds_ltd[midx])) 
                           for midx in range(len(models))]
    
    # append to events dataframe
    for sx, sub_events in enumerate(models_events):
        for mx, run_events in enumerate(sub_events):
            run_events.append(confounds_resampled[sx][mx])
    
    # append to stim_list
    [stim_list.append(x) for x in conf_keep_list]
